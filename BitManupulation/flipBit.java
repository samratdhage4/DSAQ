...
public class flipBit {
    ...
    static int bruteForce(int goal,int start)
    {
        int count=0;
    ...
    static int minBit(int start , int goal)
    {
        int count=0;
        int ans=start ^ goal;
        for(int i=0;i<31;i++)
    ...
    public static void main(String[] args) {
        ...
        System.out.println("ENTER THE START AND GOAL NUMBERS");
        int start=sc.nextInt();
        int goal=sc.nextInt();
        int ans=minBit(10, 7);
 
        System.out.println("OPTIMAL O(LOG32) :-MINIMUM BIT FLIP FOR CONVERSION OF START "+start+" GOAL "+goal+" IS "+ans);
        
        int bruteF=bruteForce(start, goal);
        System.out.println("BRUTE FORCE O(NO OF BIT) :-MINIMUM BIT FLIP FOR CONVERSION OF START "+start+" GOAL "+goal+" IS "+bruteF);
